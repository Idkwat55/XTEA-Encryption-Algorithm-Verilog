...
class IpDriver(BusDriver):
    _signals = ["clk", "rst_i", "valid_i", "en_i", "data_i",
                "key", "decrypt_i", "result_e", "valid_o", "busy_o"]
...
class SigMonitor(BusMonitor):
    def __init__(self, entity, name, clock, signal, log, callback=None, event=None):
        BusMonitor.__init__(self=self, entity=entity, name=name,
                            clock=clock, callback=callback, event=event)
        self.signal = signal
        self.clk = clock
        self.log = log
        self.name = name
...
class TB:
    def __init__(self, entity, name, clk, log):
        ...
        self.drv_data_i = IpDriver(
            self.entity, "data_i driver", self.clk, self.entity.data_i, self.log)
    ...
    def model(self, key_cipher: int, data_cipher: int):
        try:
            ...
            self.log.debug(f"[model] Encrypted Data: {[hex(encrypted[0]), hex(
                encrypted[1])]} (Combined: {hex(encrypted_result)})")

            # Decrypt the data
            decrypted = (ctypes.c_uint32 * 2)()
            lib.decipher(encrypted, decrypted, key_ctypes)
            decrypted_result = (decrypted[0] << 32) | decrypted[1]
            self.log.debug(f"[model] Decrypted Data: {[hex(decrypted[0]), hex(
                decrypted[1])]} (Combined: {hex(decrypted_result)})")
...
async def random_functional_test(dut):
    ...
    data_driver.input_key = rnd.randint(
    ...
    for i in range(0, 32):
        ...
        log.debug(f"[encryption]       raw data to dut on port data_i  : 0x{
                  temp_data:x}")
        ...
        log.debug(
            f"[encryption] [dut] processed value on port result_o: 0x{dut_val:x}")
    ...
    for i in range(0, len(tb_h.ip_encd_list)):
        log.debug(f"[decryption]       encrypted data to dut on port data_i: 0x{
                  tb_h.ip_encd_list[i]:x}")
        await data_driver._driver_send(tb_h.ip_encd_list[i])
        await data_driver._wait_for_signal(dut.valid_o)

        # Read DUT output
        await ReadOnly()
        log.debug(f"[decryption] [dut] processed value on port result_o    : {
                  hex(dut.result_o.value)}")
        tb_h.op_decd_list.append(dut.result_o.value.integer)

    log.debug("")
    # run model for generated random data (input list given to dut)
    for i in range(0, len(tb_h.ip_data_list)):
        tb_h.model(key_cipher=data_driver.input_key,
                   data_cipher=tb_h.ip_data_list[i])
...
async def directed_key_test(dut):
    ...
    for i in range(0, len(tb_h.key_list)):
        ...
        log.debug(f"[encryption]       raw key to dut on port key      : 0x{
                  data_driver.input_key:x}")
    ...
    # run model for generated random data (input list given to dut)
    for i in range(0, len(tb_h.key_list)):
        tb_h.model(key_cipher=tb_h.key_list[i],
                   data_cipher=tb_h.ip_data_list[0])
...
def start_build():
    ...
    if (log_en == "1"):
        log_run_file = f"./sim_build/{test_module}_run.log"
        log_build_file = f"./sim_build/{test_module}_build.log"
        print_col("[MAIN] Test Log Path: ./sim_build/" +
                  test_module + ".log", 0, 1)
        try:
            f = open(log_run_file, "w+")
            f.close()
        except Exception:
